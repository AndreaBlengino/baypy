...
def model_set_up(draw):
    ...
    regressors_names = np.random.choice(
        list('abcdefghijklmnopqrstuvxyz'),
        n_regressors,
        replace=False
    ).tolist()
    data = pd.DataFrame(
        {
            regressor_name: np.random.uniform(
                low=regressors_minimum,
    ...
    regressors_parameters = np.random.uniform(
        low=regressors_parameters_minimum,
        high=regressors_parameters_maximum,
        size=n_regressors + 1
    )
    response_variable = np.random.choice(
        [
            name for name in list('abcdefghijklmnopqrstuvxyz')
    ...
    priors = {
        regressor_name: {'mean': 0, 'variance': 1e14}
        for regressor_name in regressors_names
    }
    ...
    posteriors = {
        posterior_name: np.random.randn(n_samples, n_chains)
        for posterior_name in priors.keys()
    }
    posteriors['variance'] = np.abs(posteriors['variance'])

    predictors = {
        predictor: np.random.uniform(
            low=regressors_parameters_minimum,
            high=regressors_parameters_maximum,
            size=1
        )[0] for predictor in priors.keys()
...
def posteriors_data(draw):
    ...
    posterior_names = np.random.choice(
        list('abcdefghij'),
        n_posteriors,
        replace=False
    ).tolist()
...
response_variable = 'y'
data[response_variable] = 3*data['x_1'] - 20*data['x_2'] - data['x_3'] - \
    5*data['x_1 * x_2'] + 13 + 1*np.random.randn(N)
...
types_to_check = [
    ...
    {0: 1},
    None,
    pd.DataFrame(columns=['response_variable'], index=[0]),
    np.array([0])
]
