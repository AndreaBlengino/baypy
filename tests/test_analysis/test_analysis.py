...
class TestAnalysisSummary:
    ...
    def test_method(self, model_options):
        summary = bp.analysis.summary(
            model_options['posteriors'],
            quantiles=model_options['quantiles']
        )
        ...
        assert list(summary['summary'].index) == list(
            model_options['posteriors'].keys()
        )
        assert all(
            summary['summary'].columns == ['Mean', 'SD', 'HPD min', 'HPD max']
        )
        assert isinstance(summary['quantiles'], pd.DataFrame)
        assert not summary['quantiles'].empty
        assert list(summary['quantiles'].index) == list(
            model_options['posteriors'].keys()
        )
        assert all(
            summary['quantiles'].columns == [
                f'{100*quantile}%'.replace('.0%', '%')
                for quantile in model_options['quantiles']
            ]
        )

        for posterior, posterior_samples \
                in model_options['posteriors'].items():
            assert summary['summary'].loc[posterior, 'Mean'] == \
                posterior_samples.mean()
            assert summary['summary'].loc[posterior, 'SD'] == \
                posterior_samples.std()

        for posterior, posterior_samples \
                in model_options['posteriors'].items():
            for quantile in model_options['quantiles']:
                assert summary['quantiles'].loc[
                    posterior,
                    f'{100*quantile}%'.replace('.0%', '%')
                ] == np.quantile(
                    np.asarray(posterior_samples).reshape(-1), quantile
                )
    ...
    def test_raises_type_error(self, analysis_summary_type_error):
        with raises(TypeError):
            bp.analysis.summary(
                posteriors=analysis_summary_type_error['posteriors'],
    ...
    def test_raises_value_error(self, analysis_summary_value_error):
        with raises(ValueError):
            bp.analysis.summary(
                posteriors=analysis_summary_value_error['posteriors'],
...
class TestAnalysisResidualsPlot:
    ...
    def test_raises_type_error(self, analysis_residuals_plot_type_error):
        with raises(TypeError):
            bp.analysis.residuals_plot(
                model=analysis_residuals_plot_type_error
            )
    ...
    def test_raises_value_error(self, analysis_residuals_plot_value_error):
        with raises(ValueError):
            bp.analysis.residuals_plot(
                model=analysis_residuals_plot_value_error
            )
...
class TestAnalysisComputeDIC:
    ...
    def test_raises_type_error(self, analysis_compute_dic_type_error):
        with raises(TypeError):
            bp.analysis.compute_DIC(
                model=analysis_compute_dic_type_error['model'],
