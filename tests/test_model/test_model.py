...
class TestModelPriors:
    ...
    def test_property(self, model_options):
        ...
        assert all(
            [
                'mean' in regressor_data.keys() for regressor, regressor_data
                in model.priors.items() if regressor != 'variance'
            ]
        )
        assert all(
            [
                'variance' in regressor_data.keys()
                for regressor, regressor_data in model.priors.items()
        ...
        assert all(
            [
                posterior_samples.shape == (
                    model_options['n_samples'],
                    model_options['n_chains']
...
class TestModelPosteriorsToFrame:
    ...
    def test_method(self, model_options):
        ...
        assert all(
            posteriors_frame.columns == list(
                model_options['posteriors'].keys()
            )
        )
        assert len(posteriors_frame) == \
            model_options['n_samples']*model_options['n_chains']
        for col in model.posteriors.keys():
            for i in range(model_options['n_samples']):
                for j in range(model_options['n_chains']):
                    assert model.posteriors[col][i, j] == \
                        posteriors_frame.loc[
                            i*model_options['n_chains'] + j, col
                        ]
...
class TestModelPredictDistribution:
    ...
    def test_method(self, model_options):
        if len(model_options['predictors']) != 0:
            model = LinearModel()
            model.posteriors = model_options['posteriors']
            predicted = model.predict_distribution(
                predictors=model_options['predictors']
            )
    ...
    def test_raises_type_error(self, model_predict_distribution_type_error):
        with raises(TypeError):
            model.predict_distribution(
                predictors=model_predict_distribution_type_error
            )
    ...
    def test_raises_key_error(self):
        ...
        predictors = {'x': 5}
        with raises(KeyError):
            basic_model.predict_distribution(
