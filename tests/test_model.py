...
class TestModelSetData:
    ...
    def test_method(self, empty_model, general_testing_data):
        empty_model.set_data(data = general_testing_data['data'], y_name = general_testing_data['y_name'])

        assert empty_model.data.equals(general_testing_data['data'])
        assert empty_model.y_name == general_testing_data['y_name']
...
class TestModelSetPriors:
    ...
    def test_method(self, empty_model, general_testing_data):
        ...
        assert all(['mean' in regressor_data.keys() for regressor, regressor_data in empty_model.priors.items() if regressor != 'sigma2'])
        assert all(['variance' in regressor_data.keys() for regressor, regressor_data in empty_model.priors.items() if regressor != 'sigma2'])
